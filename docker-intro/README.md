#### How to use a simple docker for terraform, this is just an quick example with `-v` 'volume' mount, and `-w` 'working dir', and `--rm` to remove the conatiner after exited.
```
